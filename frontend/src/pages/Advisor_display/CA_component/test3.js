...
import Distribution_default from "./Distribution_default.js";
import Ca_dis from "./ca_dis.png";
...
const Test3 = () => {
  ...
  const handleClick = (gradePrediction, name, id, isInput) => {
    console.log(1);
    ...
    if (isInput === true) {
      ...
      console.log(gradePrediction, name);
      if (gradePrediction !== undefined) {
        setpridiction(gradePrediction);
        setName(id + " " + name);
        setisClicked(true);
      }
    }
  ...
  const backToSearch = () => {
    ...
    navigate("/course_advisor_search");
  };
      ...
      <div>
        ...
        >
          <ForceGraph2D
            ...
            onNodeClick={(node) =>
              handleClick(
                node.gradePrediction,
                node.name,
                node.id,
                node.isInput
              )
            } //Not needed for now
            // onLinkClick={(link) => handleClick(link.gradePrediction)}
            nodeRelSize={3}
            nodeColor={(node) => {
            ...
            nodeCanvasObject={(node, ctx, globalScale) => {
              const label = node.id + "\b " + "\b " + node.name;
              // const fontSize = 14 / (globalScale * 1.2);
              const fontSize = 4 * (node.val / 30);
              ...
              const re = /\b[\w']+(?:\s+[\w']+){0,1}/g;
              const lines = label.match(re);
              let x = node.x;
              let y = node.y - lineHeight * 1;
      ...
      <div className="left">
        <div className="performance">
          ...
          {isClicked === false ? (
            ...
            <Distribution Grades={pridiction} coursename={name} />
          )}
        </div>
        <img className="photo" src={Ca_dis} />
        <div className="btn_box">
          <button className="buttonCA" onClick={backToSearch}>
            Go back
          </button>
        </div>
      </div>
